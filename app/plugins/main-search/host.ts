...
import { createClipboardHistoryPlugin } from '@/app/plugins/main-search/clipboard-history-plugin'
...
const byOrderAndTitle =
  (language: AppLanguage) =>
  <T extends { manifest: { order: number; title: string } }>(a: T, b: T) => {
...
const getMainSearchPlugins = (language: AppLanguage) =>
  [
    createClipboardHistoryPlugin(language),
    createCalculatorPlugin(language),
    createTodoFocusPlugin(language),
    ...createOfficialAskPlugins(language),
  ].sort(byOrderAndTitle(language))
