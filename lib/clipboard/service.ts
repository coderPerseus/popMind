...
const sensitiveClipboardTypes = new Set(['org.nspasteboard.ConcealedType', 'org.nspasteboard.AutoGeneratedType'])

const hasSensitiveClipboardType = (items: ClipboardNativeItem[]) =>
  items.some((item) => item.types.some((typeRecord) => sensitiveClipboardTypes.has(typeRecord.type)))
...
export class ClipboardHistoryService {
  ...
  private pollClipboard = () => {
    try {
      if (selectionBridge.isClipboardFallbackActive()) {
        return
      }
      ...
      if (selectionBridge.isClipboardFallbackActive() || hasSensitiveClipboardType(items)) {
        return
      }
